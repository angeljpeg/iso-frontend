...
export function useAsignatura(asignaturaId: string) {
  ...
  const [cargaAcademica, setCargaAcademica] = useState<CargaAcademica | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const asignaturaData = await cargaAcademicaService.getAsignaturaById(asignaturaId);
