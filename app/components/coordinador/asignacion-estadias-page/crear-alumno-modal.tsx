...
import type { CreateAlumnoRequest } from "~/types/estadias/services";
import type { Estadia } from "~/types/estadias";

interface CrearAlumnoModalProps {
  isOpen: boolean;
  onClose: () => void;
  estadiaId: string;
...
export const CrearAlumnoModal = ({
  ...
  const { create, isLoading, error, clearError } = useEstadiaAlumnoActions();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const alumnoData: Omit<CreateAlumnoRequest, "token"> = {
        estadiaId,
      ...
      await create(alumnoData);
      toast.success("Alumno creado exitosamente");
      onSuccess();
    } catch (error) {
      const errorMessage =
        error instanceof Error ? error.message : "Error al crear alumno";
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
        ...
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold">Crear Nuevo Alumno</h2>
          <Button
            variant="ghost"
            size="sm"
            onClick={handleClose}
