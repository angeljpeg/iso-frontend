...
export function ProgramacionCursosTable({ 
  ...
  const [selectedSeguimiento, setSelectedSeguimiento] = useState<SeguimientoCurso | null>(null);
  ...
  const handleExportPDF = async (seguimiento: SeguimientoCurso) => {
    try {
      ...
      doc.text(`Cuatrimestre: ${seguimiento.cuatrimestre.nombre}`, 20, 60);
      ...
      if (seguimiento.detalles.length > 0) {
        ...
        seguimiento.detalles.forEach((detalle, index) => {
          ...
          if (detalle.observaciones) {
            doc.text(`Observaciones: ${detalle.observaciones}`, 25, yPosition + 24);
            yPosition += 32;
          } else {
            yPosition += 24;
          }
  ...
  if (seguimientos.length === 0) {
    return (
      <div className="text-center p-8">
        ...
        <h3 className="mt-2 text-sm font-medium text-gray-900">No hay seguimientos</h3>
              ...
              <tr key={seguimiento.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <div className="text-sm text-gray-500">
                    {seguimiento.cargaAcademica.asignatura?.clave}
                  </div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="flex items-center">
                  ...
                  <div className="flex flex-col space-y-1">
                    {seguimiento.detalles.slice(0, 3).map((detalle, index) => (
                      <Badge 
                        key={detalle.id} 
                        variant="outline" 
                        className={`${getEstadoAvanceColor(detalle.estadoAvance)} text-white text-xs`}
                      >
